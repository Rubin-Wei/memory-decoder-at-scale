...
test("exports the complete paper site for GitHub Pages", async () => {
  ...
  assert.match(html, /Faiss-based distributed high-concurrency retrieval system/);
...
test("keeps interactive source and the paper assets", async () => {
  ...
  assert.match(css, /\.figure-tech\s*{[^}]*margin-left:\s*auto[^}]*max-width:\s*360px[^}]*color:\s*var\(--blue-dark\)[^}]*line-height:\s*1\.35/s);
